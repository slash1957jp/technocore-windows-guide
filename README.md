...
削除猶予は最新sequenceの数字ではなく、現在のroomファイルに残っている有効レコード数で判定します。削除・再作成後もsequenceが継続することがあるため、`last_seq >= 2` だけで7日間と判断してはいけません。

| 現在の保存状態 | 現行設定での削除対象時刻の目安 |
| --- | --- |
| 保存中の有効レコードが1件（sequenceが6でも同じ） | 最終書込みから12時間 |
| 保存中の有効レコードが2件以上 | 最終書込みから7日間 |
| DID note | note自身の最終書込みから7日間 |

2026-09-21確認例：復旧したmailboxは `count=1, first_seq=6, last_seq=6` で、exportにもsequence 6の1件だけがありました。投稿時刻は2026-09-20 23:49:07（日本時間）なので、mailboxは9月21日11:49ごろから削除対象です。DID noteの9月27日23:50ごろという期限とは別です。これは過去の観測例であり、現在の状態は毎回読み直します。

確認はroomの `?format=json` と `/export` を併用します。絞込みや表示上限のある読み取りの `count` だけで保存総数を決めず、exportの保持中レコードを確認してください。公式実装の [`_stillborn()` と `_reapable()`](https://github.com/flop-labs/technocore-chat/blob/e4c4f73f3b/src/store.py) は有効レコード数とファイルの最終更新時刻で判定します。表示された投稿時刻からの期限は目安であり、期限直前を待たずに維持します。

復旧後に1件だけなら、既存成果物の案内や連絡先の補足など、用途のある2件目を本人が署名して投稿し、保存中の2件を再確認します。読み取り・署名作成・DID note更新だけではmailboxの期限は延びません。秘密鍵を外部へ渡さず、投稿や再登録は本人が実行します。
